...
static void	ft_get_texture(t_all *all, int x)
{
	...
	while (all->ray.drawStart <= all->ray.drawEnd)
	{
		...
		if (all->ray.side == 1)
			c = ft_color_txt(&all->txt.East, all->txt.tex_x, all->txt.tex_y);
		else if (all->ray.side == 2)
			c = ft_color_txt(&all->txt.South, all->txt.tex_x, all->txt.tex_y);
		else if (all->ray.side == 3)
			c = ft_color_txt(&all->txt.North, all->txt.tex_x, all->txt.tex_y);
		else if (all->ray.side == 4)
			c = ft_color_txt(&all->txt.West, all->txt.tex_x, all->txt.tex_y);
...
void		ft_render_sprite(t_all *all, float *dist)
{
	...
	{
		all->sprt.tex_x = (int)((all->sprt.stripe + all->sprt.sp_width\
		/ 2 - all->sprt.sp_screenX) * all->txt.Sprite.w / all->sprt.sp_width);
		if (all->sprt.transY > 0 && all->sprt.transY < dist[all->sprt.stripe])
		{
			all->sprt.y = all->sprt.start_y;
			while (all->sprt.y < all->sprt.end_y)
			{
				all->sprt.d = 128 * (all->sprt.y * 2 - all->vars.h +\
				all->sprt.sp_hight);
				all->sprt.tex_y = all->sprt.d * all->txt.Sprite.h /\
				all->sprt.sp_hight / 256;
				clr = ft_color_txt(&all->txt.Sprite, all->sprt.tex_x,\
				all->sprt.tex_y);
				if (clr)
					my_mlx_pixel_put(&all->data, all->sprt.stripe,\
					(int)all->sprt.y, clr);
				all->sprt.y += 1;
			}
		}
