...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/17 09:44:01 by cjani             #+#    #+#             */
/*   Updated: 2021/03/17 09:44:03 by cjani            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_sprite_loc(t_sprt_loc *sprites, t_all *all)
{
	int			count;
	int			j;
	int			i;
...
void			sprt_screen(t_all *all, t_sprt_loc *sp, int i)
{
	...
	all->sprt.inv = 1.0 / (all->ray.left * all->plr.dir_y - \
	all->plr.dir_x * all->ray.right);
	all->sprt.transX = all->sprt.inv * (all->plr.dir_y * all->sprt.x\
	- all->plr.dir_x * all->sprt.y);
	all->sprt.transY = all->sprt.inv * (all->ray.left * all->sprt.y -\
	all->ray.right * all->sprt.x);
	all->sprt.sp_screenX = (int)(all->vars.w - all->vars.w / 2 * (1 +\
	all->sprt.transX / all->sprt.transY));
}
...
void			ft_sprite(t_all *all, float *dist)
{
	...
	i = -1;
	ft_sprite_loc(sprites, all);
	while (++i < ft_sprite_count(all))
		sprites[i].dist = (pow((all->plr.pos_x - sprites[i].x), 2) +\
		pow((all->plr.pos_y - sprites[i].y), 2));
